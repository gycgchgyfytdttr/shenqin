...
function redzlib:MakeWindow(Configs)
	...
	local function CreateSearchBox(parent)
		...
		local SearchBoxFrame = InsertTheme(Create("Frame", SearchContainer, {
			Size = UDim2.new(1, 0, 1, 0),
			BackgroundColor3 = Theme["Color Hub 2"],
			BackgroundTransparency = 0.1
		}), "Frame")Make("Corner", SearchBoxFrame, UDim.new(0, 8))
		...
		local SearchBox = InsertTheme(Create("TextBox", SearchBoxFrame, {
		...
		SearchBoxFrame.MouseEnter:Connect(function()
			CreateTween({SearchBoxFrame, "BackgroundTransparency", 0, 0.2})
		end)
		
		SearchBoxFrame.MouseLeave:Connect(function()
			CreateTween({SearchBoxFrame, "BackgroundTransparency", 0.1, 0.2})
		end)
		
		return SearchBox, SearchContainer
	end
	...
	local function PerformSearch(searchText)
		...
		if searchText == "" or searchText == nil then
			...
			for _, tabData in pairs(redzlib.Tabs) do
				tabData.func:Visible(true)
				if tabData.func.Cont then
					tabData.func.Cont.Visible = true
				end
			end
			return
		end
		...
		-- 隐藏所有内容
		for _, tabData in pairs(redzlib.Tabs) do
			tabData.func:Visible(false)
			if tabData.func.Cont then
				tabData.func.Cont.Visible = false
			end
		end
		
		-- 搜索标签和选项
		for _, tabData in pairs(redzlib.Tabs) do
			local tabName = tabData.TabInfo.Name
			local foundInTab = false
			
			-- 检查标签名
			if string.find(string.lower(tabName), searchText, 1, true) then
				foundInTab = true
			...
			-- 检查选项
			for _, optionData in pairs(redzlib.Options) do
				if optionData.type == "Section" and string.find(string.lower(optionData.Name), searchText, 1, true) then
					foundInTab = true
					table.insert(SearchResults, {
						Type = "Section",
						Name = optionData.Name,
						Tab = tabData.func,
						Section = optionData.func
					})
				end
			end
		...
		if #SearchResults > 0 then
			Window:SelectTab(SearchResults[1].Tab)
	...
	function Window:MinimizeBtn()
		...
		if Minimized then
			...
			-- 缩小后的高度是原来的一半
			local minimizedHeight = math.floor(SaveSize.Y.Offset / 2)
			CreateTween({MainFrame, "Size", UDim2.fromOffset(SaveSize.X.Offset, minimizedHeight), 0.25, true})
			CreateTween({RainbowBorder, "Size", UDim2.new(1, 8, 1, 8), 0.25})
			Minimized = true
		end
	...
	function Window:MakeTab(paste, Configs)
		...
		-- 为每个标签添加搜索框
		local SearchBox, SearchContainer = CreateSearchBox(Container)
		SearchContainer.LayoutOrder = -999 -- 确保搜索框在最上面
		...
		function Tab:AddToggle(Configs)
			...
			-- 优化后的开关样式
			local ToggleBackground = InsertTheme(Create("Frame", Slider, {
				Size = UDim2.new(1, 0, 1, 0),
				BackgroundColor3 = Theme["Color Stroke"],
				BackgroundTransparency = 0.2
			}), "Stroke")Make("Corner", ToggleBackground, UDim.new(0.5, 0))
			...
			local function SetToggle(Val)
				...
				if Default then
					CreateTween({Toggle, "Position", UDim2.new(1, -14, 0.5), 0.25})
					CreateTween({Toggle, "BackgroundTransparency", 0, 0.25})
					CreateTween({ToggleBackground, "BackgroundColor3", Theme["Color Theme"], 0.25})
					...
					CreateTween({ToggleBackground, "BackgroundColor3", Theme["Color Stroke"], 0.25})
