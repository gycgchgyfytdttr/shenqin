...
local redzlib = {
    Themes = {
        Rainbow = {
            ...
            ["Color Theme"] = Color3.fromRGB(255, 105, 180), -- 粉色主题
            ["Color Text"] = Color3.fromRGB(255, 255, 255),
            ["Color Dark Text"] = Color3.fromRGB(220, 220, 220),
            ["Font"] = Enum.Font.Cartoon,
            ["Font Bold"] = Enum.Font.FredokaOne
        },
        Darker = {
            ["Color Hub 1"] = ColorSequence.new({
            ...
            ["Font Bold"] = Enum.Font.GothamBold
        },
        Purple = {
            ["Color Hub 1"] = ColorSequence.new({
    ...
    Save = {
        UISize = {450, 320}, -- 缩小UI尺寸
        TabSize = 140,
        Theme = "Rainbow", -- 默认使用彩虹主题
        Transparency = 0.05,
        TextFont = "Cartoon"
    },
    Settings = {},
...
local UIScale = ViewportSize.Y / 600 -- 调整缩放比例
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                ...
                if rawget(decode, "TextFont") then redzlib.Save["TextFont"] = decode["TextFont"] end
            end
        end
    end
...
local function CreateTween(Configs)
    ...
    local EasingStyle = Configs[6] or Configs.EasingStyle or Enum.EasingStyle.Quint
    local EasingDirection = Configs[7] or Configs.EasingDirection or Enum.EasingDirection.InOut
...
local function MakeDrag(Instance)
    task.spawn(function()
        ...
        local function Update(Input)
            ...
            CreateTween({Instance, "Position", Position, 0.35, false, Enum.EasingStyle.Elastic})
        end
...
-- 彩虹边框效果
local function CreateRainbowStroke(parent, props)
...
AddEle("Corner", function(parent, CornerRadius)
    local New = SetProps(Create("UICorner", parent, {
        CornerRadius = CornerRadius or UDim.new(0, 8) -- 更圆润的边角
    }), props)
    return New
end)
...
AddEle("Stroke", function(parent, props, ...)
    local args = {...}
    if redzlib.Save.Theme == "Rainbow" then
        return CreateRainbowStroke(parent, props)
    else
...
AddEle("Button", function(parent, props, ...)
    ...
    local New = InsertTheme(SetProps(Create("TextButton", parent, {
        ...
        AutoButtonColor = false,
        BackgroundTransparency = redzlib.Save.Transparency or 0.05
    }), props), "Frame")
    
    New.MouseEnter:Connect(function()
        CreateTween({New, "BackgroundTransparency", 0.2, 0.2, false, Enum.EasingStyle.Quad, Enum.EasingDirection.Out})
    end)
    New.MouseLeave:Connect(function()
        CreateTween({New, "BackgroundTransparency", redzlib.Save.Transparency or 0.05, 0.3, false, Enum.EasingStyle.Quad, Enum.EasingDirection.Out})
    end)
    if args[1] then
...
local function CreateSearchBox(parent, callback)
    local searchFrame = Create("Frame", parent, {
    ...
    searchBox.Focused:Connect(function()
        CreateTween({searchBox, "BackgroundTransparency", 0, 0.2})
        CreateTween({searchIcon, "ImageColor3", Theme["Color Theme"], 0.2})
    end)
    
    searchBox.FocusLost:Connect(function()
        CreateTween({searchBox, "BackgroundTransparency", 0.1, 0.2})
        CreateTween({searchIcon, "ImageColor3", Color3.fromRGB(200, 200, 200), 0.2})
    end)
...
-- 设备设置页面
local function CreateSettingsWindow(mainFrame)
    local settingsFrame = Create("Frame", mainFrame, {
        Size = UDim2.new(0.4, 0, 1, -40),
    ...
    Make("Corner", settingsFrame)
    Make("Stroke", settingsFrame)
    Make("Gradient", settingsFrame, {Rotation = 60})
    
    local settingsTitle = Create("TextLabel", settingsFrame, {
    ...
    local scrollFrame = Create("ScrollingFrame", settingsFrame, {
    ...
    Make("Stroke", scrollFrame)
    
    local layout = Create("UIListLayout", scrollFrame, {
        Padding = UDim.new(0, 10)
    })
    
    -- 主题选择
    local themeFrame = Create("Frame", scrollFrame, {
        Size = UDim2.new(1, 0, 0, 80),
        BackgroundTransparency = 1
    })
    
    local themeLabel = Create("TextLabel", themeFrame, {
    ...
    local themeButtons = Create("Frame", themeFrame, {
        Size = UDim2.new(1, 0, 0, 50),
        Position = UDim2.new(0, 0, 0, 25),
        BackgroundTransparency = 1
    })
    
    local buttonLayout = Create("UIListLayout", themeButtons, {
        FillDirection = "Horizontal",
        Padding = UDim.new(0, 10)
    })
    
    for themeName, _ in pairs(redzlib.Themes) do
        local themeButton = Create("TextButton", themeButtons, {
        ...
        Make("Corner", themeButton)
        Make("Stroke", themeButton)
    ...
    -- 透明度调节
    local transparencyFrame = Create("Frame", scrollFrame, {
        Size = UDim2.new(1, 0, 0, 60),
        BackgroundTransparency = 1
    })
    
    local transparencyLabel = Create("TextLabel", transparencyFrame, {
        Size = UDim2.new(1, 0, 0, 20),
        Text = "UI 透明度: " .. math.floor((1 - redzlib.Save.Transparency) * 100) .. "%",
        TextColor3 = Theme["Color Text"],
        Font = Theme["Font"],
        TextSize = 14,
        BackgroundTransparency = 1,
    ...
    local transparencySlider = Create("Frame", transparencyFrame, {
        Size = UDim2.new(1, 0, 0, 20),
        Position = UDim2.new(0, 0, 0, 25),
        BackgroundTransparency = 1
    })
    
    local sliderBar = Create("Frame", transparencySlider, {
    ...
    local sliderFill = Create("Frame", sliderBar, {
        Size = UDim2.new(1 - redzlib.Save.Transparency, 0, 1, 0),
        BackgroundColor3 = Theme["Color Theme"],
        BackgroundTransparency = 0
    })
    Make("Corner", sliderFill)
    
    local sliderButton = Create("TextButton", sliderBar, {
        Size = UDim2.new(0, 12, 0, 12),
        Position = UDim2.new(1 - redzlib.Save.Transparency, 0, 0.5, 0),
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = 0,
        Text = ""
    })
    Make("Corner", sliderButton, UDim.new(0.5, 0))
    ...
    sliderButton.MouseButton1Down:Connect(function()
        ...
        connection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            redzlib.Save.Transparency = 1 - percent
            transparencyLabel.Text = "UI 透明度: " .. math.floor(percent * 100) .. "%"
            sliderFill.Size = UDim2.new(percent, 0, 1, 0)
            sliderButton.Position = UDim2.new(percent, 0, 0.5, 0)
            
            -- 更新所有UI元素的透明度
            for _, instance in pairs(redzlib.Instances) do
                if instance.Type == "Frame" then
                    instance.Instance.BackgroundTransparency = redzlib.Save.Transparency
                end
            end
        end)
    ...
    -- 字体选择
    local fontFrame = Create("Frame", scrollFrame, {
        Size = UDim2.new(1, 0, 0, 80),
        BackgroundTransparency = 1
    })
    
    local fontLabel = Create("TextLabel", fontFrame, {
        Size = UDim2.new(1, 0, 0, 20),
        Text = "字体选择",
        TextColor3 = Theme["Color Text"],
        Font = Theme["Font"],
    ...
    local fontButtons = Create("Frame", fontFrame, {
        Size = UDim2.new(1, 0, 0, 50),
        Position = UDim2.new(0, 0, 0, 25),
        BackgroundTransparency = 1
    })
    
    local fontLayout = Create("UIListLayout", fontButtons, {
        FillDirection = "Horizontal",
        Padding = UDim.new(0, 10)
    })
    ...
    for _, fontData in pairs(fonts) do
        local fontButton = Create("TextButton", fontButtons, {
            Size = UDim2.new(0.2, 0, 1, 0),
            Text = fontData[2],
            TextColor3 = Theme["Color Text"],
            Font = Enum.Font[fontData[1]],
            TextSize = 12,
            BackgroundColor3 = Theme["Color Hub 2"],
            BackgroundTransparency = 0.1
        })
        Make("Corner", fontButton)
        Make("Stroke", fontButton)
        
        fontButton.Activated:Connect(function()
            redzlib.Save.TextFont = fontData[1]
            Theme["Font"] = Enum.Font[fontData[1]]
            Theme["Font Bold"] = fontData[1] == "Cartoon" and Enum.Font.FredokaOne or Enum.Font[fontData[1] .. "Bold"] or Enum.Font[fontData[1]]
            
            -- 更新所有文本元素的字体
            for _, instance in pairs(redzlib.Instances) do
                if instance.Type == "Text" or instance.Type == "DarkText" then
                    instance.Instance.Font = Theme["Font"]
                    if instance.Instance.Text == "UI 设置" or instance.Instance.Text == "主题颜色" or instance.Instance.Text == "UI 透明度" or instance.Instance.Text == "字体选择" then
                        instance.Instance.Font = Theme["Font Bold"]
                    end
                end
            end
...
function redzlib:SetTheme(NewTheme)
    ...
    table.foreach(redzlib.Instances, function(_,Val)
        ...
        elseif Val.Type == "Stroke" and redzlib.Save.Theme ~= "Rainbow" then
            Val.Instance[GetColor(Val.Instance)] = Theme["Color Stroke"]
        elseif Val.Type == "Theme" then
            Val.Instance[GetColor(Val.Instance)] = Theme["Color Theme"]
        elseif Val.Type == "Text" then
            Val.Instance[GetColor(Val.Instance)] = Theme["Color Text"]
            Val.Instance.Font = Theme["Font"]
...
function redzlib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "Rainbow UI Library"
    local WMiniText = Configs[2] or Configs.SubTitle or "by : 彩虹动态UI"
    ...
    -- 入场动画
    local entranceAnimation = Create("Frame", ScreenGui, {
        Size = UDim2.new(1, 0, 1, 0),
    ...
    -- 入场动画
    task.spawn(function()
        CreateTween({entranceAnimation, "BackgroundTransparency", 1, 1.5, false, Enum.EasingStyle.Quint, Enum.EasingDirection.Out})
        task.wait(0.5)
        
        MainFrame.Visible = true
        CreateTween({MainFrame, "Size", UDim2.fromOffset(UISizeX, UISizeY), 0.8, false, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out})
        CreateTween({MainFrame, "Position", UDim2.new(0.5, -UISizeX/2, 0.5, -UISizeY/2), 0.8, false, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out})
        
        task.wait(1)
        entranceAnimation:Destroy()
    end)
    ...
    -- 创建搜索框
    local searchBox = CreateSearchBox(Containers, function(searchText)
        -- 搜索功能实现
        for _, option in pairs(redzlib.Options) do
            if option.type ~= "Section" then
                local visible = string.find(string.lower(option.Name), string.lower(searchText)) ~= nil
                option.func:Visible(visible or searchText == "")
            end
        end
    end)
    
    -- 创建设置窗口
    local settingsWindow = CreateSettingsWindow(MainFrame)
    ...
    function Window:MakeTab(paste, Configs)
        ...
        function Tab:AddButton(Configs)
            ...
            FButton.Activated:Connect(function()
                ...
                CreateTween({ButtonIcon, "Rotation", 360, 0.5, false, Enum.EasingStyle.Back, Enum.EasingDirection.Out})
        ...
        function Tab:AddToggle(Configs)
            ...
            local function SetToggle(Val)
                ...
                if Default then
                    CreateTween({Toggle, "Position", UDim2.new(1, 0, 0.5), 0.25, false, Enum.EasingStyle.Elastic})
                    CreateTween({Toggle, "BackgroundTransparency", 0, 0.25})
                    CreateTween({Toggle, "AnchorPoint", Vector2.new(1, 0.5), 0.25, Wait or false})
                else
                    CreateTween({Toggle, "Position", UDim2.new(0, 0, 0.5), 0.25, false, Enum.EasingStyle.Elastic})
